...
sets = np.zeros(( len(query_points), 2, len(query_points[0]) ))
sum_query = np.zeros(( query_points.shape[0], query_points.shape[1] )) #クエリ点の合計
sum_nearest = np.zeros(( target_points.shape[0], target_points.shape[1] )) #探索した最近傍点の合計
...
# 重心の計算
centroid_query = centroid(sum_query[:3])
centroid_nearest = centroid(sum_nearest[:3])
#print( centroid(sum_query) )
#print( centroid(sum_nearest) )
